...
static Slice_(VkDeviceQueueCreateInfo) GPU_SelectVkQueueFamilies(VkPhysicalDevice physDev, VkSurfaceKHR surfaceToPresent,
...
void GPU_VkCreate(GPU_Instance* outBaseInstance, GPU_InstanceCfg cfg)
{
    #if !MSR_IOS
    {
        GPU_VK_CHECKED_CALL(volkInitialize());
    }
    #endif

    MSR_ASSERT(!!outBaseInstance && "outBaseInstance can't be null");
    outBaseInstance->base.type = GPU_GfxAPIType_Vk;
    GPU_VkInstance* output = GPU_ToVkInstance(outBaseInstance);
    ...
    GPU_VK_CHECKED_CALL(vkCreateInstance(&(VkInstanceCreateInfo)
    {
    ...
    if (validationLayersEnabled)
    {
        VkDebugUtilsMessengerCreateInfoEXT debugCreateInfo =
        {
            ...
            .pfnUserCallback = GPU_GetVkDebugCallback(),
            .pUserData       = output,
        };

        GPU_VK_CHECKED_CALL(vkCreateDebugUtilsMessengerEXT(output->instance, &debugCreateInfo, nil, &output->debugMessenger));
    }
    ...
    VkSurfaceKHR tempSurfaceForQueueSelect = VK_NULL_HANDLE;
        ...
        GPU_VK_CHECKED_CALL(vkCreateWin32SurfaceKHR(output->instance, &surfaceCreateInfo, nil, &tempSurfaceForQueueSelect));
    #elif MSR_ANDROID
        ...
        GPU_VK_CHECKED_CALL(vkCreateAndroidSurfaceKHR(output->instance, &surfaceCreateInfo, nil, &tempSurfaceForQueueSelect));
    #elif MSR_APPLE
        CAMetalLayer* tempLayer = [[CAMetalLayer alloc] init];

        GPU_VK_CHECKED_CALL(vkCreateMetalSurfaceEXT(output->instance, &(VkMetalSurfaceCreateInfoEXT)
        {
    ...
    xcb_window_t window = 0;
    {
        ...
        GPU_VK_CHECKED_CALL(vkCreateXcbSurfaceKHR(output->instance, &(VkXcbSurfaceCreateInfoKHR)
        {
    ...
    Slice_(VkDeviceQueueCreateInfo) qcis = GPU_SelectVkQueueFamilies(selectedDevice, tempSurfaceForQueueSelect,
    ...
    GPU_VK_CHECKED_CALL(vkCreateDevice(selectedDevice, &(VkDeviceCreateInfo)
    {
    ...
    GPU_VK_SET_OBJ_DEBUG_NAME(output, output->instance, "%", FMT(output->appName));
    GPU_VK_SET_OBJ_DEBUG_NAME(output, output->device, "%.device", FMT(output->appName));
    ...
    GPU_VK_SET_OBJ_DEBUG_NAME(output, output->gfxQueue, "%.gfxQueue", FMT(output->appName));
    ...
    GPU_VK_SET_OBJ_DEBUG_NAME(output, output->presQueue, "%.presQueue", FMT(output->appName));
    ...
    GPU_VK_SET_OBJ_DEBUG_NAME(output, output->dedicatedTransferQueue, "%.transferQueue", FMT(output->appName));
    ...
    GPU_VK_SET_OBJ_DEBUG_NAME(output, output->asyncComputeQueue, "%.computeQueue", FMT(output->appName));
    ...
    GPU_VK_CHECKED_CALL(vmaCreateAllocator(&(VmaAllocatorCreateInfo)
    {
...
{
    const GPU_VkInstance* renderer = GPU_ToVkInstance(baseRenderer);

    if (!renderer || !(renderer->device)) return;

    GPU_VK_CHECKED_CALL(vkDeviceWaitIdle(renderer->device));
}

void GPU_VkDestroy(GPU_Instance* baseRenderer)
{
