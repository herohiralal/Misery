...
static void GPU_Dx12GetSwapChainDimensions(GPU_Dx12SwapChain* swapChain, GPU_SwapChainCfg cfg, u32* outWidth, u32* outHeight)
{
...
static void GPU_CreateDx12SwapChain(GPU_Dx12SwapChain* swapChain, GPU_SwapChainCfg cfg)
{
    u32 width = 0, height = 0;
    GPU_Dx12GetSwapChainDimensions(swapChain, cfg, &width, &height);

    if (swapChain->actual) // already has one
    {
        GPU_DestroyDx12SwapChainImages(swapChain);
        GPU_DX12_CHECKED_CALL(swapChain->actual->ResizeBuffers(
            GPU_FRAMES_IN_FLIGHT,
            width, height,
    ...
    else
    {
        ...
        DXGI_SWAP_CHAIN_DESC1 swapChainDesc =
        {
            ...
            .BufferCount = GPU_FRAMES_IN_FLIGHT,
        ...
        GPU_DX12_CHECKED_CALL(swapChain->renderer->dxgiFactory->CreateSwapChainForHwnd(
        ...
        GPU_DX12_CHECKED_CALL(dxgiSwapChain->QueryInterface(IID_PPV_ARGS(&(swapChain->actual))));
        dxgiSwapChain->Release();

        GPU_DX12_CHECKED_CALL(swapChain->renderer->dxgiFactory->MakeWindowAssociation(
            WND_FromHandle(swapChain->window),
...
void GPU_Dx12CreateSwapChainFromWindow(GPU_SwapChain* outBaseSwapChain, GPU_Instance* baseRenderer, WND_Handle windowHandle, GPU_SwapChainCfg cfg)
{
    ...
    outBaseSwapChain->base.type = GPU_GfxAPIType_Dx12;

    GPU_Dx12SwapChain* output = GPU_ToDx12SwapChain(outBaseSwapChain);
    ...
    // fence itself
    GPU_DX12_CHECKED_CALL(renderer->device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&(output->fence))));
    GPU_DX12_SET_OBJ_DEBUG_NAME(renderer, output->fence, "%.swpch_%.fence",
    ...
    for (isize i = 0; i < GPU_FRAMES_IN_FLIGHT; i++)
    {
        ...
        baseCmdBuffer->base.type = GPU_GfxAPIType_Dx12;

        GPU_Dx12CmdBuffer* cmdBuffer = GPU_ToDx12CmdBuffer(baseCmdBuffer);
        cmdBuffer->renderer = renderer;

        GPU_DX12_CHECKED_CALL(renderer->device->CreateCommandAllocator(
            D3D12_COMMAND_LIST_TYPE_DIRECT,
            IID_PPV_ARGS(&(cmdBuffer->cmdAllocator))
        ));

        GPU_DX12_SET_OBJ_DEBUG_NAME(renderer, cmdBuffer->cmdAllocator, "%.swpch_%.cmdalloc_%",
            FMT(renderer->appName), FMT(cfg.objectName), FMT(i));

        GPU_DX12_CHECKED_CALL(renderer->device->CreateCommandList(
            0,
            D3D12_COMMAND_LIST_TYPE_DIRECT,
        ...
        GPU_DX12_SET_OBJ_DEBUG_NAME(renderer, cmdBuffer->cmdList, "%.swpch_%.cmdbuf_%",
            FMT(renderer->appName), FMT(cfg.objectName), FMT(i));
    }

    GPU_CreateDx12SwapChain(output, cfg);
}

void GPU_Dx12ReconfigureSwapChain(GPU_SwapChain* baseSwapChain, GPU_SwapChainCfg cfg)
{
    GPU_Dx12SwapChain* swapChain = GPU_ToDx12SwapChain(baseSwapChain);
    MSR_ASSERT(swapChain && "swapChain must not be null");

    GPU_Dx12WaitTillIdle(GPU_FromDx12Instance(swapChain->renderer));
    GPU_CreateDx12SwapChain(swapChain, cfg);
}

void GPU_Dx12DestroySwapChain(GPU_SwapChain* baseSwapChain)
{
    GPU_Dx12SwapChain* swapChain = GPU_ToDx12SwapChain(baseSwapChain);
    ...
    for (isize i = 0; i < GPU_FRAMES_IN_FLIGHT; i++)
    {
        GPU_Dx12CmdBuffer* cmdBuffer = GPU_ToDx12CmdBuffer(&(swapChain->buffers.cmdBuffers[i]));
...
GPU_TextureFormat GPU_Dx12GetSwapChainTextureFormat(GPU_SwapChain* baseSwapChain)
{
    GPU_Dx12SwapChain* swapChain = GPU_ToDx12SwapChain(baseSwapChain);
    if (!swapChain) return GPU_TexFmt_Unknown;

    GPU_TextureFormat output = GPU_MakeDx12TextureFormat(swapChain->swapChainFormat);
    MSR_ASSERT(output != GPU_TexFmt_Unknown && "Failed to convert DXGI_FORMAT to GPU_TextureFormat");
    return output;
}

void GPU_Dx12IterateSwapChain(GPU_SwapChain* baseSwapChain)
{
    GPU_Dx12SwapChain* swapChain = GPU_ToDx12SwapChain(baseSwapChain);
    if (!swapChain || !(swapChain->renderer))
        return;
    ...
    {
        GPU_DX12_CHECKED_CALL(swapChain->fence->SetEventOnCompletion(requiredFenceValue, swapChain->fenceEvt));
...
void GPU_Dx12PresentSwapChain(GPU_SwapChain* baseSwapChain)
{
    ...
    if (swapChain->vSync)
    {
        GPU_DX12_CHECKED_CALL(swapChain->actual->Present(1, 0));
    }
    else
    {
        GPU_DX12_CHECKED_CALL(swapChain->actual->Present(0, DXGI_PRESENT_ALLOW_TEARING));
    }

    GPU_DX12_CHECKED_CALL(swapChain->renderer->cmdQueue->Signal(swapChain->fence, swapChain->nextFenceValue + 1));
}
