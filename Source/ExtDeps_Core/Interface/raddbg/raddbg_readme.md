...
`--ipc` debugger instances), and their descriptions, are below:
 ...
 - `Step Out` (`step_out`) Runs to the first instruction after the current scope
   is exited.
 ...
 - `Selected Tab Settings` (`tab_settings`) Opens settings for a tab.
 ...
 - `Focus Menu` (`focus_menu`) Focuses the menu for the selected interface, if
   there is one.
...
A list of currently-supported views are below:
 ...
 - `slice(expr)`: Expresses that `expr` evaluates to a structure type which
   ...
   base pointer.
 - `columns(expr, ...)`: Expresses that `expr` should be expanded normally, but
