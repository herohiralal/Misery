...
static void REN_CreateVkSwapChain(REN_VkSwapChain* swapChain, REN_SwapChainCfg cfg)
{
    ...
    REN_VK_CHECKED_CALL(vkCreateSwapchainKHR(swapChain->renderer->device, &swapchainCI, nil, &(swapChain->actual)));
    ...
    REN_VK_SET_OBJ_DEBUG_NAME(swapChain->renderer, swapChain->actual, "%.swpch_%", FMT(swapChain->renderer->appName), FMT(cfg.objectName));
...
static void REN_CreateVkSwapChainImagesAndViews(REN_VkSwapChain* swapChain, REN_SwapChainCfg cfg)
{
    ...
    {
        u32 imgCount = 0;
        REN_VK_CHECKED_CALL(vkGetSwapchainImagesKHR(swapChain->renderer->device, swapChain->actual, &imgCount, nil));
...

    {
        ...
        for (isize i = 0; i < swapChain->imgs.count; i++)
        {
            REN_VK_CHECKED_CALL(vkCreateImageView(swapChain->renderer->device, &(VkImageViewCreateInfo)
            {
...
void REN_VkCreateSwapChainFromWindow(REN_SwapChain* outBaseSwapChain, REN_Instance* baseRenderer, WND_Handle windowHandle, REN_SwapChainCfg cfg)
{
    ...
    outBaseSwapChain->base.type = REN_GfxAPIType_Vk;
    REN_VkSwapChain* output = REN_ToVkSwapChain(outBaseSwapChain);
    ...
    #if MSR_WINDOWS
    {
        REN_VK_CHECKED_CALL(vkCreateWin32SurfaceKHR(renderer->instance, &(VkWin32SurfaceCreateInfoKHR)
        {
    ...
    #elif MSR_ANDROID
    {
        REN_VK_CHECKED_CALL(vkCreateAndroidSurfaceKHR(renderer->instance, &(VkAndroidSurfaceCreateInfoKHR)
        {
    ...
    {
        u32 formatCount = 0;
        REN_VK_CHECKED_CALL(vkGetPhysicalDeviceSurfaceFormatsKHR(output->renderer->physicalDevice, output->surface, &formatCount, nil));
        Slice_(VkSurfaceFormatKHR) surfaceFormats = COL_NewSlice(VkSurfaceFormatKHR, formatCount, true, MEM_temp);
        REN_VK_CHECKED_CALL(vkGetPhysicalDeviceSurfaceFormatsKHR(output->renderer->physicalDevice, output->surface, &formatCount, surfaceFormats.data));
...
void REN_VkPresentSwapChain(REN_SwapChain* baseSwapChain)
{
    ...
    REN_VK_CHECKED_CALL(vkEndCommandBuffer(cmdBuf->cmdBuffer));

    // submit command buffer
    REN_VK_CHECKED_CALL(vkQueueSubmit2(swapChain->renderer->gfxQueue, 1, &(VkSubmitInfo2)
    {
