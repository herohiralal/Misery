...
void REN_Dx12Create(REN_Instance* outBaseInstance, REN_InstanceCfg cfg)
{
    MSR_ASSERT(!!outBaseInstance && "outBaseInstance can't be null");
    outBaseInstance->base.type = REN_GfxAPIType_Dx12;

    REN_Dx12Instance* output = REN_ToDx12Instance(outBaseInstance);
    ...
    if (MSR_DBG)
    {
        REN_DX12_CHECKED_CALL(D3D12GetDebugInterface(IID_PPV_ARGS(&(output->dbgController))));
    ...
    // factory
    REN_DX12_CHECKED_CALL(CreateDXGIFactory2(dxgiFactoryFlags, IID_PPV_ARGS(&(output->dxgiFactory))));
    ...
    // device
    REN_DX12_CHECKED_CALL(D3D12CreateDevice(output->adapter, D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&(output->device))));
    REN_DX12_SET_OBJ_DEBUG_NAME(output, output->device, "%.device", FMT(output->appName));
...
void REN_Dx12WaitTillRendererIdle(REN_Instance* baseRenderer)
{
    const REN_Dx12Instance* renderer = REN_ToDx12Instance(baseRenderer);

    if (!renderer || !(renderer->device) || !(renderer->cmdQueue))
        return;

    ID3D12Fence* fence = nil;
    REN_DX12_CHECKED_CALL(renderer->device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&fence)));
    REN_DX12_CHECKED_CALL(renderer->cmdQueue->Signal(fence, 1));

    if (fence->GetCompletedValue() < 1)
    {
        HANDLE fenceEvt = CreateEventA(nil, FALSE, FALSE, nil);
        MSR_ASSERT(fenceEvt && "Failed to create fence event");
        REN_DX12_CHECKED_CALL(fence->SetEventOnCompletion(1, fenceEvt));
...
void REN_Dx12Destroy(REN_Instance* baseRenderer)
{
    REN_Dx12Instance* renderer = REN_ToDx12Instance(baseRenderer);
    if (!renderer) return;

    REN_Dx12WaitTillRendererIdle(baseRenderer);
